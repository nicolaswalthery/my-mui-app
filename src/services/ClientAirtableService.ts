...
export class ClientAirtableService {
  private static instance: ClientAirtableService;
  ...
  public static getInstance(): ClientAirtableService {
    if (!ClientAirtableService.instance) {
      ClientAirtableService.instance = new ClientAirtableService();
    }
    return ClientAirtableService.instance;
  }
  ...
  public async createClient(clientData: ClientData): Promise<ClientData> {
    try {
      ...
      console.log("airtableFields:"+ airtableFields.Nom)
      console.log("airtableFields:"+ airtableFields.Prénom)
      console.log("airtableFields:"+ airtableFields['Source Enregistrement'])
  ...
  public async updateClientByEmail(currentEmail: string, clientData: ClientData): Promise<ClientData> {
    try {
      ...
      const requestData: AirtableUpdateRequest = {
        records: [
          {
  ...
  /**
   * Delete a client record by email
   */
  public async deleteClientByEmail(email: string): Promise<boolean> {
    try {
      const existingRecord = await this.findRecordByEmail(email);
      ...
      await axiosInstance.delete(`/${this.tableName}?records[]=${existingRecord.id}`);
      return true;
    } catch (error: any) {
